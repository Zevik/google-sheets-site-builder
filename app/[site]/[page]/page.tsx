...
export async function generateMetadata({ params }: { params: { site: string, page: string } }): Promise<Metadata> {
  const data = await getSiteAndPageData(params.site, params.page);
...
// דף ספציפי באתר
export default async function SitePageContent({ params }: { params: { site: string, page: string } }) {
  const data = await getSiteAndPageData(params.site, params.page);
